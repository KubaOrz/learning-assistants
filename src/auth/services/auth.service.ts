...
export class AuthService {
    ...
    async authenticateUser(loginRequest: AuthenticationRequest): Promise<AuthenticationResponse> {
        const user = await this.userService.findUserForAuthentication(loginRequest.email);
        if (!user) {
    ...
    async createNewUser(createUserRequest: CreateUserRequest): Promise<AuthenticationResponse> {
        const userAlreadyExists = await this.userService.userExists(createUserRequest.email);
        if (userAlreadyExists) {
            throw new ConflictException(`User with email ${createUserRequest.email} already exists`);
        }
